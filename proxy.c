...
void doit(int first_fd)
{
    ...
    if (end_fd == -1){
        // 캐시에 요청한 파일이 있을 때 -> 송이가 알아서 처리함+                                            
        printf("나는 송이, 알아서 처리해볼거임\n", n); // 답장을 중선에게 주겠음
        ...
        Rio_writen(first_fd, srcp, sbuf.st_size); /* 파일을 클라이언트에 전달 */
        free(srcp);
    }
    else if (end_fd < 0)
    {
        ...
        if (filename == '/home.html'){
            cache_fd = Open(filename, O_RDWR, 777);
            while ((n = Rio_readlineb(&rio2, buf, MAXLINE)) != 0)
            {                                                                      
              printf("나는 송이, %d바이트를 받고 중선이한테 그대~로 넘겨줌", n); 
              Rio_writen(first_fd, buf, n);
              Rio_writen(cache_fd, buf, n);
            }
            Close(cache_fd);
        }
        else
